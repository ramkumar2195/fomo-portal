...
/**
 * Pill styling for a follow-up status. Previously rendered as inline
 * plain-text ("• SCHEDULED •") inside the history modals which was
 * hard to scan against the dense surrounding text. Now each status
 * gets a coloured pill so it pops at first glance:
 *   SCHEDULED  → sky    (action expected)
 *   COMPLETED  → emerald (finished, no action)
 *   MISSED     → amber  (overdue / abandoned)
 *   CANCELLED  → slate  (closed out)
 */
function followUpStatusPillClass(status?: string): string {
  const s = (status || "").toUpperCase();
  if (s === "COMPLETED") return "border-emerald-300 bg-emerald-100 text-emerald-800";
  if (s === "MISSED") return "border-amber-300 bg-amber-100 text-amber-800";
                  ...
                  return (
                    <div key={entry.followUpId} className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700">
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        ...
                        <span
                          className={`inline-flex rounded-full border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider ${followUpStatusPillClass(entry.status)}`}
                        >
                          {entry.status || "SCHEDULED"}
                        </span>
                      </div>
                      <p className="mt-1 text-xs font-medium text-slate-500">
                        {(entry.channel || "CALL").replace(/_/g, " ")} · Next {formatDateTimeDisplay(entry.dueAt)}
                      </p>
                    ...
                    <div key={history.followUpId} className="rounded-md border border-slate-200 bg-white px-2 py-1.5">
                      <div className="flex flex-wrap items-center gap-2">
                        <p className="text-xs font-semibold text-slate-800">
                          {formatResponseTypeLabel(history.responseType)}
                        </p>
                        <span
                          className={`inline-flex rounded-full border px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wider ${followUpStatusPillClass(history.status)}`}
                        >
                          {history.status || "SCHEDULED"}
                        </span>
                        <span className="text-[11px] text-slate-500">
                          {new Date(history.dueAt).toLocaleString("en-IN")}
                        </span>
                      </div>
